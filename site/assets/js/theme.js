import { t, getAvailableLangs, getLanguageLabel, getCurrentLang, switchLanguage, ensureLanguageBundle } from './i18n.js?v=press-system-v3.4.125';
...
function ensurePressComponents() {
  ...
  try {
    if (!registry[COMPONENTS_READY]) {
      registry[COMPONENTS_READY] = import('./components.js?v=press-system-v3.4.125').catch((err) => {
    ...
    return import('./components.js?v=press-system-v3.4.125').catch((err) => {
      console.warn('[theme] Failed to load press components', err);
      return null;
    });
  }
